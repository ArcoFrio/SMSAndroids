...
**[SaveManager.cs](SMSAndroids/SaveManager.cs)** — thin façade over the pack store plus the after-sleep gameplay turnover. Static `Get/Set{String,Int,Float,Bool}` + `HasVariable` route through `ModForgeBridge` (packId `SMSAndroidsPack`), so existing call sites keep working while persistence lives entirely in the pack (the pack manifest declares every variable; numeric bounds like Affection 0–5 are enforced there). Still owns: the post-sleep turnover at the `afterSleepEvents`→`savedUI` gate — now only a GC2 proxy refresh, `HarborHome_Slept` (which reads plugin-side `Places.harborHomeBedroomSwapApplied`) and the gift-shop texture; **no** disk writes, ModForge commits — and the one-time legacy `.txt` importer. The gift-shop counter increment was removed: the pack's `GS_Counter` rule owns that count, and this was a second unlatched writer racing it. `HarborHome_SleepCD` is `refreshMode: Daily` in the manifest instead of being cleared here. (Voyeur-tier rebuild removed — pack-owned.) No more `defaultValues`, cache, file I/O, slot management, or NanoSave save-slot listeners (those moved to ModForge's `PackManualSaveSync`).
...
**[MainStory.cs](SMSAndroids/MainStory.cs)** — shared `SignalArgs` constants, and nothing else. The `relaxed` / `actionTodaySB` per-day flags are gone: nothing ever read them and no proxy mirror existed, and the pack owns the real ones (`SecretBeach_RelaxedAmount`, `SecretBeach_ActionToday`). The old per-character dialogue dispatcher is gone — pack dialogues with conditions/actions replaced it. **The voyeur system is fully pack-owned** (eligible-target List variable + AddToList/RemoveFromList maintenance, DailyRandom lottery variable, "Random from list" pick); `VoyeurBridge` and all tier/pick code were removed. The three general lottery numbers are gone too — the pack's ScheduleDaily rules roll their own DailyChance.
...
- **Numeric bounds are pack-declared.** `Affection_*` carry `minValue`/`maxValue` in the manifest and `PackVariableStore` clamps on write. `SaveManager.SetInt` does NOT clamp — it is a one-line forward to the bridge. Add bounds in the pack, not here.
