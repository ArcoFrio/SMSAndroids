...
2. **`Characters.loadedBusts`** — needs `loadedCore` + **waits for ModForge's bust pass** (probes `bustManager.Find("AnisBase")`). Resolves the 21 character + 15 vanilla bust fields by name; builds Anis Harbor-Home NPC slots.
...
- Bundles: `BepInEx\plugins\SMSAndroidsCore\Assets\{characterbundle,dialoguebundle,otherbundle,minigamebundle}`. `dialoguebundle` survives mainly for the **Proxy Variables** GNV asset; `characterbundle` for Anis HH NPC prefabs; `otherbundle` for UI sounds + `ButtonTemplate`; `minigamebundle` for the massage minigame.
...
**[SaveManager.cs](SMSAndroids/SaveManager.cs)** — thin façade over the pack store plus the after-sleep gameplay turnover. Static `Get/Set{String,Int,Float,Bool}` + `HasVariable` route through `ModForgeBridge` (packId `SMSAndroidsPack`), so existing call sites keep working while persistence lives entirely in the pack (the pack manifest declares every variable; numeric bounds like Affection 0–5 are enforced there). Still owns: the post-sleep turnover at the `afterSleepEvents`→`savedUI` gate (lottery re-rolls, gift-shop counter, schedule refresh — **no** disk writes; ModForge commits), and the one-time legacy `.txt` importer. (Voyeur-tier rebuild removed — pack-owned.) No more `defaultValues`, cache, file I/O, slot management, or NanoSave save-slot listeners (those moved to ModForge's `PackManualSaveSync`).

**[Schedule.cs](SMSAndroids/Schedule.cs)** — per-character location simulation. `SetDay{1..5}Schedule`, `UpdateScheduleForDay`, `scheduleVersion`; public API `GetCharacterLocation` / `SetCharacterLocation` / `AnyCharacterLocationStartsWith` (SMSAndroids-internal — packs that need to drive schedule do it generically by setting a pack variable this code reads, not via any ModForge-side schedule action); Harbor-Home roaming (per-character timers, slot rules, no-share constraints); Anis HH NPC variant gating per frame; shower-audio gating; mirrors locations to `Location_<Char>` proxies.

**[Places.cs](SMSAndroids/Places.cs)** — vanilla-level cache + SMSAndroids-side extras grafted onto **pack-built** levels. `TryResolvePackBuiltPlaces()` binds `secretBeach*`/`mountainLab*`/`giftShop*`/`harborHome*` level + roomtalk fields by name-suffix (retry-gated until ModForge builds them). Extras it still owns: HH NPC slot Transforms, B1 overlays (shower glass / fridge), bedroom `PlayerRoom_ButtonCanvas` graft, SecretBeach Sky/Flash/Gatekeeper/Portal + Solid cameo, weather toggles, bedroom swap after sleeping at HH, gift shop texture by build status, **mod shops/GiftStore** (`ActivateShop`, `AddItemToGiftStore`, visibility by `Gift_*` proxies), `CreateNewRoomTalk` (Hospital/Hotel/Trail extras), "House for Sale"→"Home" label swap on the **pack-built** Foundry radial button. **No map buttons, no navigation, no navigator grid** — all ModForge (`NavigatorRuntime` evaluates the pack's per-button conditions; `NavigatorGridSetup`/`NavigatorGridLayout` own the 6-column grid + extra nav row).

**[Characters.cs](SMSAndroids/Characters.cs)** — bust field resolution (by GO name, from ModForge-built busts) + Anis HH NPC factory (`CreateNPC` from `characterbundle`) + `characterGiftLikesMap` + blink/fade/particle helpers used by the minigame.

**[Dialogues.cs](SMSAndroids/Dialogues.cs)** — Gift UI (`InitializeGiftUICanvas`, `AddGiftItem`, per-frame visibility), HHTalk panel (per-room character buttons), `audioShower`/`audioShowerQuiet`/`audioHarborHomeMusic` handles, `dialoguePlaying`/`dialoguePlayingVanilla` flags (mirrored to `Checks_Dialogue-is-playing`). Each frame, `dialoguePlaying = ModForgeBridge.IsAnyDialoguePlaying() || dialoguePlayingVanilla` — **we read ModForge's generic query; ModForge no longer writes our fields.** All dialogue *content* lives in the pack.

**[MainStory.cs](SMSAndroids/MainStory.cs)** — slim story state: shared `SignalArgs` constants, three general lottery numbers, per-day flags (`relaxed`, `actionTodaySB`). The old per-character dialogue dispatcher is gone — pack dialogues with conditions/actions replaced it. **The voyeur system is fully pack-owned** (eligible-target List variable + AddToList/RemoveFromList maintenance, DailyRandom lottery variable, "Random from list" pick); `VoyeurBridge` and all tier/pick code were removed.
...
`BreastPhysics`, `DragSpriteController`, `LotionTrail`, `SqueezeContourFollower`, `SqueezeSpriteController` (shader-driven interaction effects; shaders in repo root `Sprites_*.shader`), `RandomChildActivator` (Anis NPC pose variants), `MassageMovementPattern` (minigame data POCOs), `TransformExtensions`.

### Removed in the migration (don't recreate)

`Scenes.cs`, `Wallpaper.cs`, `BustOutfitDescriptor.cs`, `BustPacks.cs`, `GridLayoutGroup.cs`, `ButtonHover.cs`, MainStory's dialogue runner, Dialogues' prefab inventory + SFX-text system + `{var}` substitution, Places' `CreateNewPlace`/`CreateNewLevel`/map-button gating/`ClickMapButton`. Their replacements live in SMSModForge.PackPlugin + the pack manifest.
