...
namespace SMSAndroidsCore
{
    public class GridLayoutGroup : UnityEngine.UI.GridLayoutGroup
    {
        ...
        {
            // Source code by MrBeardy: https://github.com/mrbeardy/BeardyGridLayout/blob/main/src/Runtime/GridLayoutGroup.cs
            ...
            // --- BEGIN: Vertical shift so last row is always at the top, but only if more than one row ---
            float verticalShift = (actualCellCountY > 1) ? (actualCellCountY - 1.5f) * (cellSize.y + spacing.y) : 0f;
            startOffset.y -= verticalShift;
            // --- END: Vertical shift ---
            
            // Defer the extraNavRow activation/deactivation to avoid layout rebuild conflicts
            if (Places.extraNavRow != null)
            {
                bool shouldBeActive = actualCellCountY > 1;
                if (Places.extraNavRow.activeSelf != shouldBeActive)
                {
                    // Use MonoBehaviour.StartCoroutine to defer the operation
                    if (this is MonoBehaviour mb)
                    {
                        mb.StartCoroutine(DeferSetActive(Places.extraNavRow, shouldBeActive));
                    }
                }
            }
            ...
            // Apply the same vertical shift to lastCellsStartOffset
